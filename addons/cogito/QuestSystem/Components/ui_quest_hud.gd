...
@onready
var completed_group: VBoxContainer = $QuestDisplay/VBoxContainer/TabContainer/QUESTS_completed
...
func _on_quest_completed(_passed_quest: CogitoQuest):
	_on_set_quest_notification(
		null, str(tr("QUEST_complete") + ": " + tr(_passed_quest.quest_title))
	)
