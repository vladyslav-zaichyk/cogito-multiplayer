...
func update_quest():
	CogitoGlobals.debug_log(
		true, "cogito_quest_updater.gd", "Attempting to update quest " + quest_to_update.quest_name
	)
	match update_type:
		UpdateType.Start:
		...
		UpdateType.Fail:
			CogitoGlobals.debug_log(
				true, "cogito_quest_updater.gd", "Failing quest " + quest_to_update.quest_name
			)
...
func _on_pickup_component_was_interacted_with(
	_interaction_text: Variant, _input_map_action: Variant
) -> void:
	update_quest()
