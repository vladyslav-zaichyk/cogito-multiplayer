	...
	if (
		(
			...
			in [
				...
				"open_rotation",
				"closed_rotation",
				"forced_rotation_axis",
				"angle_tolerance"
			]
		)
		and door_type != DoorType.ROTATING
	):
		...
		(
			...
			in [
				"animation_player",
				"opening_animation",
				"reverse_opening_anim_for_close",
				"closing_animation"
			]
		)
		and door_type != DoorType.ANIMATED
	):
		property.usage = PROPERTY_USAGE_NO_EDITOR
		...
		if is_moving:
			match forced_rotation_axis:
				ForcedRotationAxis.None:
					...
					if vectors_approx_equal(
						target_rotation_vector, rotation_degrees, angle_tolerance
					):
						is_moving = false
				...
				ForcedRotationAxis.AxisX:
					rotation.x = lerp_angle(
						...
						deg_to_rad(target_rotation_vector.x),
						clampf(_delta * door_speed, 0.0, 1.0)
					)
				...
				ForcedRotationAxis.AxisY:
					rotation.y = lerp_angle(
						...
						deg_to_rad(target_rotation_vector.y),
						clampf(_delta * door_speed, 0.0, 1.0)
					)
				...
				ForcedRotationAxis.AxisZ:
					rotation.x = lerp_angle(
						...
						deg_to_rad(target_rotation_vector.z),
						clampf(_delta * door_speed, 0.0, 1.0)
					)
		...
		for slot_data in inventory.inventory_slots:
			if slot_data != null and slot_data.inventory_item == key:
				player_interaction_component.send_hint(
					null, tr(key.name) + " " + tr("HINT_SWITCH_item_used")
				)  # Sends a hint with the key item name.
