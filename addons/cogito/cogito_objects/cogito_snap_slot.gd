...
func _ready() -> void:
	...
	if expected_object:
		...
		CogitoGlobals.debug_log(
			true,
			"cogito_snap_slot.gd",
			"expected object cogito_name=" + instanced_expected_object.cogito_name
		)
...
func place_carryable(world_carryable: Node3D):
	...
	if !world_carryable.is_class("RigidBody3D"):
		CogitoGlobals.debug_log(
			true, "cogito_snap_slot.gd", "place_carryable(): body wasn't a RigidBody3D."
		)
		return
...
func _on_body_entered_snap_area(body: Node3D):
	...
	if body is CogitoObject:
		CogitoGlobals.debug_log(
			true, "cogito_snap_slot.gd", "body is CogitoObject with cogito_name=" + body.cogito_name
		)
		if instanced_expected_object.cogito_name == body.cogito_name:
			CogitoGlobals.debug_log(
				true, "cogito_snap_slot.gd", "Expected object detected: " + body.cogito_name
			)
