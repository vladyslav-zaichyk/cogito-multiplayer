...
enum DespawningLogic {
	NONE = 0, ONLY_EMPTY = 1, ONLY_EMPTY_POST_INTERACTION = 2, ONLY_TIMED = 4, EMPTY_AND_TIMED = 8  ## Container will not despawn and will become part of the scene.  ## Container will despawn only after player takes out the last item in it.  ## Container will despawn only after player takes out the last item in it. This option will delay container deletion until player stops interacting with the container. Even if player puts an item in the container, container is deleted upon closing of the external_inventory.  ## Container will despawn after a preset time interval started upon container creation. Note: Items present will be lost with timed despawning logic.  ## Container will despawn after a time period as well as upon a timer expiration. Note: Items present will be lost with timed despawning logic.
}

const DESPAWNINGLOGICTYPES: Array = [
...
func _ready() -> void:
	if debug_prints:
		...
		(
			...
			. debug_log(
				debug_prints,
				"Loot Component/Loot Drop Container",
				(
					"Debug prints bool is true. Randomizing container despawn types. Despawning Logic is set to: "
					+ str(despawning_logic)
				)
			)
		)
...
func _set_up_timer():
	if timer == null:
		...
		CogitoGlobals.debug_log(
			debug_prints,
			"Loot Component/Loot Drop Container",
			"Initial Spawn: " + str(initial_spawn) + "Timer State: " + str(timer.is_stopped())
		)
		...
		CogitoGlobals.debug_log(
			...
			"Loot Component/Loot Drop Container",
			(
				"timer NOT found and time left set is: "
				+ str(timer.wait_time)
				+ " | Timer state: "
				+ str(timer.is_stopped())
			)
		)
		...
		CogitoGlobals.debug_log(
			debug_prints,
			"Loot Component/Loot Drop Container",
			"set_up_timer was ran but timer already existed during call with these settings: "
		)
		CogitoGlobals.debug_log(
			debug_prints, "Loot Component/Loot Drop Container", "Timer ID: " + str(timer)
		)
		CogitoGlobals.debug_log(
			debug_prints,
			"Loot Component/Loot Drop Container",
			"Timer One Shot?: " + str(timer.one_shot)
		)
		CogitoGlobals.debug_log(
			debug_prints,
			"Loot Component/Loot Drop Container",
			"Timer Wait Time: " + str(timer.wait_time)
		)
		CogitoGlobals.debug_log(
			debug_prints,
			"Loot Component/Loot Drop Container",
			"Timer Start Time: " + str(start_time)
		)
		CogitoGlobals.debug_log(
			debug_prints, "Loot Component/Loot Drop Container", "Timer End Time: " + str(end_time)
		)
	...
	if timer != null:
		if !initial_spawn:
			if time_passes_when_unloaded:
				...
				CogitoGlobals.debug_log(
					debug_prints, "Loot Component/Loot Drop Container", "timer is: " + str(timer)
				)
				CogitoGlobals.debug_log(
					debug_prints,
					"Loot Component/Loot Drop Container",
				...
				if current_time < end_time:
					...
					CogitoGlobals.debug_log(
						debug_prints,
						"Loot Component/Loot Drop Container",
						"Current time is smaller than end time. _time is set to: " + str(_time)
					)
					...
					(
						...
						. debug_log(
							debug_prints,
							"Loot Component/Loot Drop Container",
							"Allotted time for existence has passed. Setting _time to 3.0 to initiate despawn."
						)
					)
			else:
				if time_left > 0.0:
...
func _on_despawn_timer_timeout():
	if _clear_for_despawning():
		...
		CogitoGlobals.debug_log(
			debug_prints, "Loot Component/Loot Drop Container", "Despawning Container: " + str(self)
		)
		_remove_container()
	else:
		CogitoGlobals.debug_log(
			debug_prints,
			"Loot Component/Loot Drop Container",
			"Container: " + str(self) + " is unable to despawn."
		)
		return
	...
	if inventory_slot_data != null:
		...
		if items_in_slots.is_empty():
			if despawning_logic == DespawningLogic.ONLY_EMPTY:
				...
				despawning_logic == DespawningLogic.ONLY_EMPTY_POST_INTERACTION
				or despawning_logic == DespawningLogic.EMPTY_AND_TIMED
			):
				marked_for_deletion = true
				CogitoGlobals.debug_log(
					debug_prints,
					"Loot Component/Loot Drop Container",
					"Container is now marked for deletion. Will process after inventory closed."
				)
	...
	if !despawning_logic == DespawningLogic.NONE:
		if marked_for_deletion:
			CogitoGlobals.debug_log(
				debug_prints,
				"Loot Component/Loot Drop Container",
				"Despawning marked container: " + str(self)
			)
			_remove_container()

		if (
			despawning_logic == DespawningLogic.ONLY_TIMED
			or despawning_logic == DespawningLogic.EMPTY_AND_TIMED
		):
			if timer != null and timer.paused:
				...
				CogitoGlobals.debug_log(
					debug_prints,
					"Loot Component/Loot Drop Container",
					"Paused timer of: " + str(self) + " was unpaused."
				)
		...
		if _player_hud.inventory_interface.is_inventory_open:
			...
			if timer != null:
				...
				CogitoGlobals.debug_log(
					...
					"Loot Component/Loot Drop Container",
					(
						"Unpausing timer. Time Remaining: "
						+ str(timer.time_left)
						+ " "
						+ str(time_left)
						+ " "
	...
	if timer != null:
		if timer.time_left > 0:
			...
			CogitoGlobals.debug_log(
				debug_prints,
				"Loot Component/Loot Drop Container",
				"Time left before despawning: " + str(time_left)
			)
