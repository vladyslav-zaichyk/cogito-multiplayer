...
@onready var footstep_interval_change_velocity_square: float = (
	footstep_interval_change_velocity * footstep_interval_change_velocity
)
...
func _ready():
	...
	for attribute in find_children("", "CogitoAttribute", false):
		...
		CogitoGlobals.debug_log(
			is_logging, "cogito_player.gd", "Cogito Attribute found: " + attribute.attribute_name
		)
	...
	for currency in find_children("", "CogitoCurrency", false):
		...
		CogitoGlobals.debug_log(
			is_logging, "cogito_player.gd", "Cogito Currency found: " + currency.currency_name
		)
	...
	CogitoGlobals.debug_log(
		is_logging, "cogito_player.gd", "Player has no reference to pause menu."
	)
...
func increase_attribute(
	attribute_name: String, value: float, value_type: ConsumableItemPD.ValueType
) -> bool:
	var attribute = player_attributes.get(attribute_name)
	if not attribute:
		CogitoGlobals.debug_log(
			is_logging, "cogito_player.gd", "Increase attribute: Attribute not found"
		)
		return false
...
func decrease_attribute(attribute_name: String, value: float):
	...
	if not attribute:
		CogitoGlobals.debug_log(
			is_logging,
			"cogito_player.gd",
			"Decrease attribute: " + attribute_name + " - Attribute not found"
		)
		return
...
func increase_currency(currency_name: String, value: float) -> bool:
	...
	if not currency:
		CogitoGlobals.debug_log(
			is_logging, "cogito_player.gd", "Increase currency: Currency not found"
		)
		return false
	else:
		currency.add(value)
...
func decrease_currency(currency_name: String, value: float):
	...
	if not currency:
		CogitoGlobals.debug_log(
			is_logging,
			"cogito_player.gd",
			"Decrease currency: " + currency_name + " - Currency not found"
		)
		return
...
func _reload_options():
	...
	if err == 0:
		...
		HEADBOBBLE = config.get_value(
			OptionsConstants.section_name, OptionsConstants.head_bobble_key, 1
		)
		MOUSE_SENS = config.get_value(
			OptionsConstants.section_name, OptionsConstants.mouse_sens_key, 0.25
		)
		INVERT_Y_AXIS = config.get_value(
			OptionsConstants.section_name, OptionsConstants.invert_vertical_axis_key, true
		)
		TOGGLE_CROUCH = config.get_value(
			OptionsConstants.section_name, OptionsConstants.toggle_crouching_key, true
		)
		JOY_H_SENS = config.get_value(
			OptionsConstants.section_name, OptionsConstants.gp_looksens_key, 2
		)
		JOY_V_SENS = config.get_value(
			OptionsConstants.section_name, OptionsConstants.gp_looksens_key, 2
		)
...
func handle_sitting_look(event):
	...
	var target_direction = (
		Vector2(look_marker_position.x - neck_position.x, look_marker_position.z - neck_position.z)
		. normalized()
	)
	...
	if not (
		new_angle_to_marker > 180 - sittable_look_angle
		and new_angle_to_marker < (180 + sittable_look_angle)
	):
	...
	if sittable:
		...
		if sittable.physics_sittable:
			...
			tween.tween_property(
				self,
				"global_transform",
				sittable.sit_position_node.global_transform,
				sittable.tween_duration
			)
...
func _sit_down_finished():
	...
	if sittable_look_marker:
		...
		tween.tween_property(
			neck, "global_transform:basis", target_transform.basis, sittable.rotation_tween_duration
		)
...
func _move_to_original_position(sittable):
	...
	tween.tween_property(
		neck, "global_transform:basis", original_neck_basis, sittable.rotation_tween_duration
	)
...
func _move_to_leave_node(sittable):
	...
	if sittable.leave_node_path:
		...
		if leave_node:
			var tween = create_tween()
			tween.tween_property(
				self, "global_transform", leave_node.global_transform, sittable.tween_duration
			)
			tween.tween_property(
				neck,
				"global_transform:basis",
				original_neck_basis,
				sittable.rotation_tween_duration
			)
			tween.tween_callback(Callable(self, "_stand_up_finished"))
		else:
			CogitoGlobals.debug_log(
				true, "CogitoProjectile", "No leave node found. Returning to Original position"
			)
			_move_to_original_position(sittable)
...
func _move_to_nearby_location(sittable):
	CogitoGlobals.debug_log(
		true, "CogitoPlayer", "Attempting to find available locations to move player to"
	)
	...
	while attempts < max_attempts:
		...
		var random_direction = (
			Vector3(randf_range(-0.1, 0.1), randf_range(-0.1, 0.1), randf_range(-0.1, 0.1))  # Degree of Y random actually makes this work better at finding candidates
			. normalized()
		)
	...
	CogitoGlobals.debug_log(
		true,
		"CogitoPlayer",
		"No available location found after " + str(attempts) + " tries. Testing for leave node."
	)
	_move_to_leave_node(sittable)
...
func test_motion(transform3d: Transform3D, motion: Vector3) -> bool:
	return PhysicsServer3D.body_test_motion(
		self_rid, get_params(transform3d, motion), test_motion_result
	)
...
func _process_on_ladder(_delta):
	...
	if (
		CAN_SPRINT_ON_LADDER
		and Input.is_action_pressed("sprint")
		and input_dir.length_squared() > 0.1
	):
		is_sprinting = true
	...
	direction = (
		(body.global_transform.basis * Vector3(input_dir.x, input_dir.y * y_dir, 0)).normalized()
	)
...
func _process_on_sittable(delta):
	...
	if joystick_h_event:
		if abs(joystick_h_event.get_axis_value()) > JOY_DEADZONE:
			...
			head.rotation.x = clamp(
				head.rotation.x,
				deg_to_rad(-sittable.horizontal_look_angle),
				deg_to_rad(sittable.horizontal_look_angle)
			)
...
func _physics_process(delta):
	...
	if is_on_floor():
		...
		if was_in_air and last_velocity.y < landing_threshold:
			...
			var velocity_ratio = clamp(
				(
					(last_velocity.y - min_landing_velocity)
					/ (max_landing_velocity - min_landing_velocity)
				),
				0.0,
				1.0
			)
	...
	if (
		crouched_jump
		or (not jumped_from_slide and is_on_floor() and try_crouch or crouch_raycast.is_colliding())
	):
		...
		if (
			...
			and stamina_attribute
			and stamina_attribute.value_current > 0
		):
	...
	if not is_on_floor():
		...
		eyes.position.y = lerp(
			eyes.position.y, wiggle_vector.y * (wiggle_current_intensity / 2.0), delta * LERP_SPEED
		)
		eyes.position.x = lerp(
			eyes.position.x, wiggle_vector.x * wiggle_current_intensity, delta * LERP_SPEED
		)
	else:
		if !is_movement_paused or !is_showing_ui:
	...
	if (
		...
		is_jumping = true
		is_in_air = false
		var doesnt_need_stamina = (
			not stamina_attribute
			or stamina_attribute.value_current >= stamina_attribute.jump_exhaustion
		)
	...
	if sliding_timer.is_stopped():
		...
		direction = (
			(body.global_transform.basis * Vector3(slide_vector.x, 0.0, slide_vector.y))
			. normalized()
		)
...
func step_check(delta: float, is_jumping_: bool, step_result: StepResult):
	...
	if main_velocity.y >= 0:
		for i in range(STEP_CHECK_COUNT):
			...
			var test_motion_params: PhysicsTestMotionParameters3D = (
				PhysicsTestMotionParameters3D.new()
			)
			test_motion_params.from = transform3d
			test_motion_params.motion = motion

			var is_player_collided: bool = PhysicsServer3D.body_test_motion(
				self.get_rid(), test_motion_params, test_motion_result
			)
			...
			is_player_collided = PhysicsServer3D.body_test_motion(
				self.get_rid(), test_motion_params, test_motion_result
			)
			...
			if not is_player_collided:
				...
				if is_player_collided:
					if (
						test_motion_result.get_collision_normal().angle_to(Vector3.UP)
...
func apply_external_force(force_vector: Vector3):
	if force_vector and force_vector.length() > 0:
		CogitoGlobals.debug_log(
			is_logging, "cogito_player.gd", "Applying external force " + str(force_vector)
		)
