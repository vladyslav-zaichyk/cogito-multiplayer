...
enum SitAreaBehaviour { MANUAL, AUTO, NONE }  ## Player needs to interact manually  ## Player sits automatically on entry  ## Player can interact from outside Sit Area

enum PlacementOnLeave { ORIGINAL, AUTO, TRANSFORM, DISPLACEMENT }  ## Player returns to original location  ## Attempt to find nearby available location for Player using Navmesh - Experimental  ## Player is placed at defined Leave node  Make sure this is setup in Nodes section  ## The displacement between player and Sittable is taken on sit down, and used on standup relative to Sittables new location
...
func _ready():
	...
	if not sit_position_node:
		CogitoGlobals.debug_log(
			true, "CogitoSittable", "Sit position node not found. Ensure the path is correct."
		)
	else:
		displace_sit_marker()
