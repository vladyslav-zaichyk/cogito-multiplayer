...
func _calculate_timer_value():
	calculated_respawn_timer = (
		(respawn_timer_days * 86400.0)
		+ (respawn_timer_hours * 3600.0)
		+ (respawn_timer_minutes * 60.0)
		+ respawn_timer_seconds
	)

	CogitoGlobals.debug_log(
		debug_prints,
		"Loot Component/Lootable Container",
		"Calculated respawn timer is: " + str(calculated_respawn_timer)
	)
...
func _populate_the_container(_inventory: CogitoInventory, _items: Array[LootDropEntry]):
	...
	if slots.size() > 0:
		...
		if _player_hud != null and viewing_this_container:
			CogitoGlobals.debug_log(
				debug_prints,
				"Loot Component/Lootable Container",
				"Player Hud found and is viewing this container:" + str(self)
			)
	...
	CogitoGlobals.debug_log(
		debug_prints,
		"Loot Component/Lootable Container",
		"Inventory size set to: " + str(slots.size())
	)
	...
	for i in slots:
		CogitoGlobals.debug_log(
			debug_prints,
			"Loot Component/Lootable Container",
			"Slot number: " + str(_index) + " holds item: " + str(slots[_index])
		)
		_index += 1
...
func _set_up_timer():
	if timer == null:
		...
		CogitoGlobals.debug_log(
			debug_prints, "Loot Component/Lootable Container", "Timer ID: " + str(timer)
		)
		CogitoGlobals.debug_log(
			debug_prints,
			"Loot Component/Lootable Container",
		...
		CogitoGlobals.debug_log(
			debug_prints, "Loot Component/Lootable Container", "Timer End Time: " + str(end_time)
		)
	...
	if timer != null:
		if !initial_spawn:
			if time_passes_when_unloaded:
				...
				CogitoGlobals.debug_log(
					debug_prints, "Loot Component/Lootable Container", "timer is: " + str(timer)
				)
				CogitoGlobals.debug_log(
					debug_prints,
					"Loot Component/Lootable Container",
				...
				if current_time < end_time:
					...
					(
						...
						. debug_log(
							debug_prints,
							"Loot Component/Lootable Container",
							"Allotted time for existence has passed. Setting _time to 3.0 to initiate respawn."
						)
					)
			else:
				if time_left > 0.0:
...
func _on_inventory_hidden():
	if viewing_this_container:
		CogitoGlobals.debug_log(
			debug_prints,
			"Loot Component/Lootable Container",
			"Inventory Hidden signal receieved. Clearing viewing_this_container variable."
		)
		if viewing_this_container:
...
func _on_inventory_toggled(external_inventory_owner: Node):
	if (
		self == external_inventory_owner
		and inventory_respawning_logic == InventoryRespawningLogic.TIMED_RESPAWN
	):
		if _player_hud.inventory_interface.is_inventory_open:
			_pause_timer()
			...
			CogitoGlobals.debug_log(
				debug_prints,
				"Loot Component/Lootable Container",
				"Pausing timer. Time Remaining: " + str(timer.time_left)
			)
			...
			CogitoGlobals.debug_log(
				debug_prints,
				"Loot Component/Lootable Container",
				"Unpausing timer. Time Remaining: " + str(timer.time_left)
			)
