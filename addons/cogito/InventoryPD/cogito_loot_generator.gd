...
func _sort_loot_table(loot_table: LootTable):
		...
		for index in loot_to_generate:
			match index.droptype:
				0:
					none.append(index)
					push_warning(
						"Loot table contains items with drop type set to none, these items will not be dropped."
					)
				1:
	...
	CogitoGlobals.debug_log(
		debug_prints,
		"Loot Component/Loot Generator",
		(
			"None Size: "
			+ str(none.size())
			+ str(none)
			+ " Always Size: "
			+ str(guaranteed_drops_table.size())
			+ str(guaranteed_drops_table)
			+ " Chance Size: "
			+ str(chance_drops_table.size())
			+ str(chance_drops_table)
			+ " Quest Size: "
			+ str(quest_drops_table.size())
			+ str(quest_drops_table)
		)
	)
...
func _count_quest_items(item: InventoryItemPD) -> int:
	...
	for _slot in _lookup_merge:
		if _slot == item:
			_count += 1
	CogitoGlobals.debug_log(
		debug_prints, "Loot Component/Loot Generator", "Quest items count is: " + str(_count)
	)
	return _count
...
func _roll_for_randomized_items(_items: Array[LootDropEntry], _amount: int) -> Array[LootDropEntry]:
	...
	if _items.is_empty():
		(
			...
			. debug_log(
				debug_prints,
				"Loot Component/Loot Generator",
				"_roll_for_randomized_items(): Passed item array was empty! Returning empty lootdrop array..."
			)
		)
	...
	while _result.size() < _amount:
		...
		# Handle Quest Items
			...
			CogitoGlobals.debug_log(
				...
				(
					"Quest ID: "
					+ str(_winner.quest_id)
					+ " Quest Item Total Count: "
					+ str(_winner.quest_item_total_count)
				)
			)
			if _count_quest_items(_winner.inventory_item) >= _winner.quest_item_total_count:  # Check inventory and already spawned loot bags for a count and compare to maximum allowed count
				CogitoGlobals.debug_log(
					debug_prints,
					"Loot Component/Loot Generator",
					"Maximum amount of quest items reached. Moving on."
				)
				...
				if CogitoQuestManager.active.get_ids_from_quests().has(_winner.quest_id):
					if _result.has(_winner):  # Check if the final array already has this quest item waiting for drop
						if not (
							_result.count(_winner) + _count_quest_items(_winner.inventory_item)
							>= _winner.quest_item_total_count
						):  # Check if new copy wouldn't go over the maximum limit
							_result.append(_winner)
						CogitoGlobals.debug_log(
							debug_prints,
							"Loot Component/Loot Generator",
							"You've got a quest item in loot generator."
						)
					else:  # Final array does not have a copy of this quest item awating drop
						if not (
							_count_quest_items(_winner.inventory_item)
							>= _winner.quest_item_total_count
						):  # Final check for the maximum limit
							_result.append(_winner)
	...
	if debug_prints:
		CogitoGlobals.debug_log(
			debug_prints,
			"Loot Component/Loot Generator",
			"Amount of items in results array: " + str(_result.size())
		)
		CogitoGlobals.debug_log(
			debug_prints, "Loot Component/Loot Generator", "Array contains these items:"
		)
		for i in _result:
		...
		CogitoGlobals.debug_log(
			debug_prints,
			"Loot Component/Loot Generator",
			"Array took " + str(_failsafe) + " tries to complete."
		)
