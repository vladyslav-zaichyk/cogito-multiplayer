...
enum EnemyState { IDLE, PATROLLING, AWARE, CHASING, DEAD }
...
func handle_patrolling(_delta: float):
	if !patrol_path:
		CogitoGlobals.debug_log(
			true, "cogito_basic_enemy.gd", "No patrol path found. Switching to idle."
		)
		switch_to_idle()
		return
	...
	if !is_waiting:
		if patrol_path.patrol_points.size() <= 0:
			CogitoGlobals.debug_log(
				true, "cogito_basic_enemy.gd", "Patrol points array is empty. Switching to idle."
			)
			switch_to_idle()
			return
		if (
			global_position.distance_to(
				patrol_path.patrol_points[patrol_point_index].global_position
			)
			< patrol_point_threshold
		):
			is_waiting = true
			...
			var look_ahead := Vector3(
				global_position.x + velocity.x, global_position.y, global_position.z + velocity.z
			)
...
func load_patrol_points():
	if patrol_path_nodepath:
		CogitoGlobals.debug_log(
			true, "cogito_basic_enemy.gd", "Loading patrol path: " + str(patrol_path_nodepath)
		)
