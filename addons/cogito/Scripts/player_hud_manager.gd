...
@onready
var ui_attribute_area: BoxContainer = $MarginContainer_BottomUI/PlayerAttributes/MarginContainer/VBoxContainer
...
func connect_to_external_inventories():  # Grabbing external inventories in scene.
	for node in get_tree().get_nodes_in_group("external_inventory"):
		CogitoGlobals.debug_log(
			true, "player_hud_manager", node.name + " is in external_inventory group."
		)
...
func _is_steam_deck() -> bool:
	if (
		RenderingServer.get_rendering_device().get_device_name().contains("RADV VANGOGH")
...
func _on_update_wieldable_data(
	passed_wieldable_item: WieldableItemPD,
	passed_ammo_in_inventory: int,
	passed_ammo_item: AmmoItemPD
):
	if passed_wieldable_item:
		wieldable_hud.show()
		wieldable_hud.update_wieldable_data(
			passed_wieldable_item, passed_ammo_in_inventory, passed_ammo_item
		)
