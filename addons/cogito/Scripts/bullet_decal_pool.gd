...
static func spawn_bullet_decal(
	global_pos: Vector3,
	normal: Vector3,
	parent: Node3D,
	bullet_basis: Basis,
	...
	if len(decal_pool) >= MAX_BULLET_DECALS and is_instance_valid(decal_pool[0]):
		...
		decal_instance = (
			preload("res://addons/cogito/Assets/VFX/Impacts/bullet_decal.tscn").instantiate()
		)
	...
	decal_instance.global_transform = (
		Transform3D(bullet_basis, global_pos)
		* Transform3D(Basis().rotated(Vector3(1, 0, 0), deg_to_rad(90)), Vector3())
	)
	# Align to surface
	decal_instance.global_basis = (
		Basis(Quaternion(decal_instance.global_basis.y, normal)) * decal_instance.global_basis
	)
