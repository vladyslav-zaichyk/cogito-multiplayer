...
#	Processing Inventory Items for Localization
	if res is InventoryItemPD:
		...
		ret.append(
			PackedStringArray(
				[inventory_item.description, msgtxt, msgid_plural, "item description"]
			)
		)
		ret.append(
			PackedStringArray(
				[inventory_item.hint_text_on_use, msgtxt, msgid_plural, "item hint on use"]
			)
		)
		...
		if inventory_item is WieldableItemPD:
			ret.append(
				PackedStringArray(
					[inventory_item.hint_on_empty, msgtxt, msgid_plural, "item hint on empty"]
				)
			)
			ret.append(
				PackedStringArray(
					[inventory_item.ammo_item_name, msgtxt, msgid_plural, "item ammo item name"]
				)
			)
...
#	Processing Quest Resources for Localization
	if res is CogitoQuest:
		...
		ret.append(
			PackedStringArray(
				[quest.quest_description_active, msgtxt, msgid_plural, "quest description active"]
			)
		)
		ret.append(
			PackedStringArray(
				[
					quest.quest_description_completed,
					msgtxt,
					msgid_plural,
		...
		ret.append(
			PackedStringArray(
				[quest.quest_description_failed, msgtxt, msgid_plural, "quest description failed"]
			)
		)
...
#	Processing Translation Key Dictionaries for Localization
	if res is TranslationKeyDict:
		...
		for key_dict_entry in translation_key_dict.keylist:
			...
			ret.append(
				PackedStringArray(
					[key_dict_entry, msgtxt, msgid_plural, "translation_key_dict entry"]
				)
			)
