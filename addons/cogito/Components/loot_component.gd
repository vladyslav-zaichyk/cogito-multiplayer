	...
	if spawning_logic == SpawningLogic.SPAWN_ITEM:
		...
		(
			...
			. debug_log(
				...
				"Loot Component",
				(
					"Spawning Logic for this loot component is set to None. Component will not function. Parent: "
					+ str(get_parent())
				)
			)
		)
				...
				if children.size() > 0:
					for child in children:
						if child is PickupComponent:
							child.slot_data.quantity = randi_range(
								item.quantity_min, item.quantity_max
							)  # adjust quantity based on the loot table data.
		...
		for item in spawned_items:
			...
			if children.size() > 0:
				for child in children:
					...
					if child is PickupComponent:
						if (
							child.slot_data.inventory_item.name != null
							and child.slot_data.quantity > 1
						):
							item.display_name = str(
								(
									child.slot_data.inventory_item.name
									+ " x"
									+ str(child.slot_data.quantity)
								)
							)
...
func _spawn_loot_container():
	...
	if enabled and amount_of_items_to_drop > 0:
		...
		CogitoGlobals.debug_log(
			...
			(
				"Spawned Loot Bag: "
				+ str(spawned_loot_bag)
				+ "at these coordinates: "
				+ str(spawned_loot_bag.position)
			)
		)
...
func _populate_the_container(_inventory: CogitoInventory, _items: Array[LootDropEntry]):
	...
	CogitoGlobals.debug_log(
		debug_prints, "Loot Component", "Inventory size set to: " + str(slots.size())
	)
