...
func start_reaction_threshold_timer(passed_collider: Node3D):
	...
	if !passed_collider.cogito_properties:
		CogitoGlobals.debug_log(
			is_logging,
			"cogito_properties.gd",
			"Collider " + passed_collider.name + " has no properties."
		)
		return
...
func check_for_reaction_timer_interrupt(passed_collider: Node3D):
	...
	if passed_collider == reaction_collider:
		print(
			get_parent().name,
			": Passed collider and current reaction collider match! Reaction timer is stopped is ",
			reaction_timer.is_stopped()
		)
		if !reaction_timer.is_stopped():
			print(
				get_parent().name,
				" has stopped reaction timer as the collider ",
				passed_collider,
	...
	if reaction_collider.cogito_properties:
		...
		if reaction_collider.cogito_properties.is_on_fire:
			if elemental_properties & ElementalProperties.FLAMMABLE:
				print(
					get_parent().name,
					": Touched burning collider ",
					reaction_collider.name,
					". I'm flammable, igniting self."
				)
				set_on_fire()
			else:
				print(
					...
					": Touched collider ",
					reaction_collider.name,
					" which is on fire, but I'm not flammable, so nothing happens."
				)
			if elemental_properties & ElementalProperties.WET:
				print(
					get_parent().name,
					": Touched burning collider ",
					reaction_collider.name,
					". I'm wet so I'll extinguish the collider."
				)
		...
		match reaction_collider.cogito_properties.elemental_properties:
			ElementalProperties.WET:  ### WATER REACTIONS
				if is_on_fire:
					print(
						get_parent().name,
						": Touched by wet collider ",
						reaction_collider.name,
						" while on fire. Extinguishing self."
					)
					extinguish()
					make_wet()
				else:
					print(
						...
						": Touched by wet collider ",
						reaction_collider.name,
						" but wasn't on fire. Getting wet."
					)
					make_wet()
...
func set_on_fire():
	if elemental_properties & ElementalProperties.FLAMMABLE:
		...
		CogitoGlobals.debug_log(
			is_logging, "cogito_properties.gd", get_parent().name + " is not FLAMMABLE."
		)
...
func clear_spawned_effects():
	for node in spawned_effects:
		CogitoGlobals.debug_log(
			is_logging, "cogito_properties.gd", "Clearing out spawned effect " + node.name
		)
		node.queue_free()
...
func make_electric():
	if elemental_properties & ElementalProperties.CONDUCTIVE:
		...
		CogitoGlobals.debug_log(
			is_logging, "cogito_properties.gd", get_parent().name + " has become electric."
		)
		...
		CogitoGlobals.debug_log(
			is_logging, "cogito_properties.gd", get_parent().name + " is not CONDUCTIVE."
		)
	...
	if reaction_bodies.size() < 1:
		(
			...
			. debug_log(
				...
				(
					get_parent().name
					+ " systemic properties: Can't recheck reactions as there's no reaction bodies stored."
				)
			)
		)
		return
