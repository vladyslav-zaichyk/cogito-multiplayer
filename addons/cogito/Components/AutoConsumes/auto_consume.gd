...
func initialize_auto_consume() -> void:
	if !consumables or consumables.size() == 0:
		CogitoGlobals.debug_log(
			true,
			"AutoConsume",
			"No consumables added to the " + attribute_name + " attribute's auto-consume list."
		)
		return
	...
	if consume_if_increases_max:
		...
		inventory.connect(
			"picked_up_new_inventory_item", Callable(self, "_on_picked_up_new_inventory_item")
		)
...
func _on_player_state_loaded() -> void:
	...
	if consume_if_increases_max:
		...
		var inventory: CogitoInventory = (
			(get_parent() as CogitoPlayer).inventory_data as CogitoInventory
		)
		inventory.connect(
			"picked_up_new_inventory_item", Callable(self, "_on_picked_up_new_inventory_item")
		)
...
func _on_attribute_changed(
	_attribute_name: String, _value_current: float, _value_max: float, has_increased: bool
) -> void:
	if consume_if_increases_max and _value_max > last_value_max:
		...
		consume_on_increase
		and has_increased
		and _value_current != _value_max
		and _value_current > threshold_value
	):
		_auto_consume(false)
...
func _auto_consume(value_max_increased: bool, remainder: float = 0.0) -> void:
	var inventory_slots: Array[InventorySlotPD] = (
		(get_parent() as CogitoPlayer).inventory_data.inventory_slots
	)
	for inventory_slot in inventory_slots:
		...
		if inventory_slot.inventory_item is ConsumableItemPD:
			for consumable in consumables:
				if (
					consumable.name != inventory_slot.inventory_item.name
					or consumable.attribute_name != attribute_name
				):
					continue
				if consumable.attribute_change_amount == 0.0:  # Shouldn't occur
					continue
				if (
					consumable.value_to_change == ConsumableEffect.ValueType.MAX
					and !value_max_increased
				):
					continue
				...
				if remainder > 0.0 and consumable.attribute_change_amount <= remainder:
					CogitoGlobals.debug_log(
						true, "AutoConsume", consumable.name + " isn't effective enough to be used."
					)
					continue
...
func _on_picked_up_new_inventory_item(slot_data: InventorySlotPD) -> void:
	...
	for consumable in consumables:
		if (
			consumable.name != slot_data.inventory_item.name
			or consumable.attribute_name != attribute_name
		):
			continue
...
func _on_attribute_reached_zero(
	_attribute_name: String, _value_current: float, _value_max: float
) -> void:
	_auto_consume(false)
	_update_values()
...
func _consume_item(slot_data: InventorySlotPD) -> void:
	...
	CogitoGlobals.debug_log(
		true,
		"AutoConsume",
		"Auto-consumed " + slot_data.inventory_item.name + " for " + attribute_name
	)

	if hint_text_on_use != "":
		(get_parent() as CogitoPlayer).player_interaction_component.send_hint(
			hint_icon_on_use, hint_text_on_use + " " + str(slot_data.inventory_item.name)
		)
