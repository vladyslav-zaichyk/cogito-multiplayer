...
func _process(delta):
	...
	if (
		player.is_sprinting
		and player.current_speed > player.WALKING_SPEED
		and player.velocity.length() > 0.1
	):
		regen_timer.stop()
		is_regenerating = false
	...
	if (
		!is_regenerating
		and regen_timer.is_stopped()
		and value_current < value_max
...
func _run_exhaustion() -> float:
	...
	if slope_factor < 0.0001:  # Running uphill
		var uphill_run_exhaustion: float = lerpf(
			run_exhaustion_speed, uphill_run_exhaustion_max_speed, abs(slope_factor)
		)
		#print("uphill exhaustion speed is ", uphill_run_exhaustion)
		return uphill_run_exhaustion
	else:  # Running downhill
		var downhill_run_exhaustion: float = lerpf(
			run_exhaustion_speed, downhill_run_exhaustion_max_speed, slope_factor
		)
