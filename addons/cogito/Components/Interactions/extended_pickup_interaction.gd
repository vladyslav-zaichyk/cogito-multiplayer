...
func _ready() -> void:
	...
	if (
		!pickup
		or pickup is not PickupComponent
		or !pickup.slot_data
		or !pickup.slot_data.inventory_item
	):
		CogitoGlobals.debug_log(
			true,
			"ExtendedPickupInteraction",
			"ExtendedPickupInteraction can't find a pickup component"
		)
		is_disabled = true
		return
	...
	if item is ConsumableItemPD:
		...
		if player.player_attributes.has(consumable.attribute_name):
			var attribute: CogitoAttribute = (
				player.player_attributes[consumable.attribute_name] as CogitoAttribute
			)
		...
		is_disabled = (
			player_interaction_component.is_carrying
			or !player_interaction_component.player.inventory_data.can_pick_up_slot_data(
				pickup.slot_data
			)
		)
...
func cant_reload_current_wieldable() -> bool:
	if player_interaction_component.equipped_wieldable_node.animation_player.is_playing():
		return true
	if (
		player_interaction_component.equipped_wieldable_item.ammo_item_name
		!= pickup.slot_data.inventory_item.name
	):
		return true
	...
	var ammo_needed: int = abs(
		(
			player_interaction_component.equipped_wieldable_item.charge_max
			- player_interaction_component.equipped_wieldable_item.charge_current
		)
	)
...
func attempt_reload_current_wieldable(ammo: AmmoItemPD) -> void:
	...
	player_interaction_component.equipped_wieldable_item.update_wieldable_data(
...
func attempt_wield(wieldable: WieldableItemPD) -> void:
	...
	if player_interaction_component.player.inventory_data.pick_up_slot_data(pickup.slot_data):
		...
		player_interaction_component.send_hint(
			wieldable.icon, wieldable.name + " couldn't be picked up."
		)
#endregion
