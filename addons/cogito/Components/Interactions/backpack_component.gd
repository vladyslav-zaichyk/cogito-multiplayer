...
func update_player_inventory(player: CogitoPlayer):
	...
	player_inventory.inventory_slots.resize(
		player_inventory.inventory_size.x * player_inventory.inventory_size.y
	)
	...
	for slot_data in inventory.inventory_slots:
		if slot_data != null:
			...
			CogitoGlobals.debug_log(
				true,
				"BackpackComponent",
				slot_data.inventory_item.name + " added to temp_item_array."
			)
...
func give_back_items(_passed_inventory: CogitoInventory) -> void:
	...
	for item in temp_item_array:
		...
		CogitoGlobals.debug_log(
			true,
			"BackpackComponent",
			item.inventory_item.name + " added  back to player inventory."
		)
