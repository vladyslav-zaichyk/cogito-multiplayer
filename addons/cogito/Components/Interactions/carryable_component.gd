...
func _physics_process(_delta):
	if is_being_carried:
		...
		carry_position = player_interaction_component.get_carryable_destination_point(
...
func rotate_object(_delta):
	...
	if input_dir.length() > 0:
		...
		var rotation_basis: Basis = camera.global_basis.rotated(
			camera.global_basis.x, -camera.global_rotation.x
		)
		var rotation_vector: Vector3 = (
			rotation_basis * Vector3(input_dir.y, input_dir.x, 0).normalized()
		)
