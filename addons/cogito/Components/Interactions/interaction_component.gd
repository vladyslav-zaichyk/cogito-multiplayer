...
func check_attribute(player_interaction_component: PlayerInteractionComponent) -> bool:
	...
	fetched_attribute = player_interaction_component.player.player_attributes.get(
		attribute_to_check
	)
	...
	if fetched_attribute == null:
		CogitoGlobals.debug_log(
			...
			"attribute_to_check (" + attribute_to_check + ") was not found in player_attributes!"
		)
		return false
	...
	CogitoGlobals.debug_log(
		...
		(
			...
			+ attribute_to_check
			+ "): Min value needed = "
			+ str(min_value_to_pass)
			+ ". Current value = "
			+ str(fetched_attribute.value_current)
		)
	)
	...
	if fetched_attribute.value_current >= min_value_to_pass:
		if message_on_pass and attribute_check == AttributeCheck.FAIL_MESSAGE:
			player_interaction_component.send_hint(
				fetched_attribute.attribute_icon, message_on_pass
			)
		...
		if message_on_pass and attribute_check == AttributeCheck.FAIL_MESSAGE:
			player_interaction_component.send_hint(
				fetched_attribute.attribute_icon, message_on_fail
			)
		return false
