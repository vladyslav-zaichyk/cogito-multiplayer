...
const HSliderWLabel = preload("res://addons/cogito/easy_menus/Scripts/slider_w_labels.gd")
...
@onready
var fullscreen_resolution_current_value_label: Label = %FullscreenResolutionCurrentValueLabel
@onready
var h_box_container_fullscreen_resolution: HBoxContainer = %HBoxContainer_FullscreenResolution
...
func save_options():
	config.set_value(
		OptionsConstants.section_name,
		OptionsConstants.invert_vertical_axis_key,
		invert_y_check_button.button_pressed
	)
	config.set_value(
		OptionsConstants.section_name,
		OptionsConstants.toggle_crouching_key,
		toggle_crouching_check_button.button_pressed
	)
	config.set_value(
		OptionsConstants.section_name, OptionsConstants.head_bobble_key, headbob_strength
	)
	...
	config.set_value(
		OptionsConstants.section_name, OptionsConstants.fullscreen_mode_key_name, is_fullscreen()
	)
	config.set_value(
		OptionsConstants.section_name,
		OptionsConstants.resolution_index_key_name,
		windowed_resolution_option_button.selected
	)
	config.set_value(
		OptionsConstants.section_name,
		OptionsConstants.fullscreen_resolution_scale_key,
		fullscreen_resolution_slider.value / 100.0
	)
	config.set_value(
		OptionsConstants.section_name, OptionsConstants.gui_scale_key, gui_scale_slider.value
	)
	config.set_value(
		OptionsConstants.section_name, OptionsConstants.vsync_key, vsync_check_button.button_pressed
	)
	config.set_value(
		OptionsConstants.section_name,
		OptionsConstants.msaa_2d_key,
		anti_aliasing_2d_option_button.get_selected_id()
	)
	config.set_value(
		OptionsConstants.section_name,
		OptionsConstants.msaa_3d_key,
		anti_aliasing_3d_option_button.get_selected_id()
	)
	...
	config.set_value(
		OptionsConstants.section_name,
		OptionsConstants.sfx_volume_key_name,
		sfx_volume_slider.hslider.value
	)
	config.set_value(
		OptionsConstants.section_name,
		OptionsConstants.music_volume_key_name,
		music_volume_slider.hslider.value
	)

	# SAVING INPUT MAP
	var serialized_inputs = InputHelper.serialize_inputs_for_actions()
	config.set_value(
		OptionsConstants.key_binds, OptionsConstants.input_helper_string, serialized_inputs
	)
...
func load_options(skip_applying: bool = false):
	...
	if !have_cfg:
		(
			...
			. debug_log(
				true,
				"OptionsTabMenu.gd",
				"Loading options config failed (likely first run). Using project defaults until user applies settings."
			)
		)

	var invert_y = config.get_value(
		OptionsConstants.section_name, OptionsConstants.invert_vertical_axis_key, true
	)
	var toggle_crouching = config.get_value(
	...
	gp_looksens = config.get_value(
		OptionsConstants.section_name, OptionsConstants.gp_looksens_key, 2
	)
	headbob_strength = config.get_value(
		OptionsConstants.section_name, OptionsConstants.head_bobble_key, 2
	)
	var fullscreen_mode = config.get_value(
		OptionsConstants.section_name, OptionsConstants.fullscreen_mode_key_name, is_fullscreen()
	)
	...
	var fullscreen_resolution_scale = config.get_value(
		OptionsConstants.section_name, OptionsConstants.fullscreen_resolution_scale_key, 1.0
	)
	var gui_scale = config.get_value(
		OptionsConstants.section_name, OptionsConstants.gui_scale_key, 1
	)
	...
	var sfx_volume = config.get_value(
		OptionsConstants.section_name, OptionsConstants.sfx_volume_key_name, 1
	)
	var music_volume = config.get_value(
		OptionsConstants.section_name, OptionsConstants.music_volume_key_name, 1
	)
	...
	match headbob_strength:
		1:
			headbob_option_button.selected = 0
		3:
			headbob_option_button.selected = 1
		7:
			headbob_option_button.selected = 2
	if !skip_applying:
...
func load_keybindings_from_config():
	...
	if err != 0:
		CogitoGlobals.debug_log(
			true, "OptionsTabMenu.gd", "Keybindings: Loading options config failed."
		)
		#save_keybindings_to_config()

	var serialized_inputs = config.get_value(
		OptionsConstants.key_binds, OptionsConstants.input_helper_string, serialized_default_inputs
	)
