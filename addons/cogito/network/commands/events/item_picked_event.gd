...
## Event emitted when a player picks up an item.

var item: InventoryItemPD
var slot_data: InventorySlotPD


func _init(player_id_value: int = -1, item_value: InventoryItemPD = null, slot_data_value: InventorySlotPD = null):
	super._init(player_id_value, "item_picked")
	item = item_value
...
static func deserialize(data: Dictionary) -> Event:
	...
	return ItemPickedEvent.new(player_id, item, slot_data)
