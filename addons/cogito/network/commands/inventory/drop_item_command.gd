...
static func deserialize(data: Dictionary) -> Command:
	...
	if slot_data_dict.has("item"):
		...
		if item_data.has("resource_path") and not item_data["resource_path"].is_empty():
			...
			if not resource_path.ends_with(".gd"):
				...
				if not item:
					push_warning("DropItemCommand: Failed to load item from resource_path: %s, trying fallback" % resource_path)
			else:
				push_warning("DropItemCommand: resource_path points to script (.gd), trying fallback: %s" % resource_path)
		...
		if not item:
			...
			push_error("DropItemCommand: Failed to deserialize slot_data - cannot load item '%s' (resource_path missing or invalid)" % item_name)
			return null
	else:
		# Item data missing
		push_error("DropItemCommand: Failed to deserialize slot_data - item data missing")
		return null
	...
	if item:
		...
		push_error("DropItemCommand: Failed to deserialize slot_data - item is null after loading")
		return null
