...
func _ready() -> void:
	...
	if not head_node:
		push_error("NetworkRotationSync: Could not find head node")
		return
...
func _process(delta: float) -> void:
	if not parent_body or not body_node or not head_node:
		return
...
func _send_rotation_update() -> void:
	...
	if body_diff < rotation_threshold and head_diff < rotation_threshold:
		return
	...
	if multiplayer:
		_sync_rotation_rpc.rpc(current_body_rotation, current_head_rotation)


## RPC: Sync rotation to all clients
@rpc("any_peer", "call_local", "unreliable")
func _sync_rotation_rpc(body_rotation: float, head_rotation: float) -> void:
...
func _interpolate_rotation(delta: float) -> void:
	...
	# Interpolate head rotation (X-axis)
...
func force_sync_rotation(body_rotation: float, head_rotation: float) -> void:
	...
	if is_local and multiplayer:
		_sync_rotation_rpc.rpc(body_rotation, head_rotation)
