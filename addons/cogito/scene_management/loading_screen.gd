...
func _ready():
	...
	ResourceLoader.load_threaded_request(
		next_scene_path, "", false, ResourceLoader.CACHE_MODE_IGNORE
	)  # Start loading the next scene


func _process(_delta):
	if (
		ResourceLoader.load_threaded_get_status(next_scene_path)
		...
		if load_mode != 2:  #Checking that load_mode is not set to 2 which would ignore scene states.
			...
			if load_mode == 1:  #Load_Mode one is attempting to load a save
				CogitoGlobals.debug_log(
					true,
					"loading_screen.gd",
					"Attempting to load a save for passsed_slot " + passed_slot
				)
