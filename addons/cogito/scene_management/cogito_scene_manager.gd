...
func switch_active_slot_to(slot_name: String):
	...
	if !_player_state:
		CogitoGlobals.debug_log(
			true, "CSM", "Existing player state for slot " + slot_name + " not found."
		)
...
func get_existing_player_state(passed_slot) -> CogitoPlayerState:
	var player_state_file: String = (
		cogito_state_dir + passed_slot + "/" + cogito_player_state_prefix + ".res"
	)
	CogitoGlobals.debug_log(true, "CSM", "Looking for file: " + player_state_file)
	if ResourceLoader.exists(player_state_file):
		CogitoGlobals.debug_log(
			true, "CSM", "CSM: Get existing player state: found for slot " + passed_slot
		)
		return ResourceLoader.load(player_state_file, "", ResourceLoader.CACHE_MODE_IGNORE)
	else:
		CogitoGlobals.debug_log(
			true, "CSM", "Get existing player state: No player state found for slot " + passed_slot
		)
		return null
...
func get_existing_scene_state(passed_slot) -> CogitoSceneState:
	...
	var scene_state_file: String = (
		cogito_state_dir
		+ passed_slot
		+ "/"
		+ CogitoGlobals.scene_state_prefix
		+ current_scene
		+ ".res"
	)
	...
	if ResourceLoader.exists(scene_state_file):
		CogitoGlobals.debug_log(
			true, "CSM", "Get existing scene state: found for slot " + passed_slot
		)
		return ResourceLoader.load(scene_state_file, "", ResourceLoader.CACHE_MODE_IGNORE)
	else:
		CogitoGlobals.debug_log(
			true, "CSM", "Get existing scene state: No scene state found for slot " + passed_slot
		)
		return null
...
func loading_saved_game(passed_slot: String, current_scene_name: String = ""):
	...
	if _current_scene_name == _player_state.player_current_scene:
		...
		CogitoGlobals.debug_log(
			true, "CSM", "Player state for slot " + passed_slot + " is from current scene."
		)
		...
		CogitoGlobals.debug_log(
			...
			"CSM",
			(
				"Player state for slot "
				+ passed_slot
				+ " is in different scene ("
				+ _player_state.player_current_scene
				+ "). Transitioning..."
			)
		)
		load_next_scene(
			_player_state.player_current_scene_path, "", passed_slot, CogitoSceneLoadMode.LOAD_SAVE
		)
...
func load_player_state(player, passed_slot: String):
	...
	if _player_state and _player_state.state_exists(passed_slot):
		CogitoGlobals.debug_log(
			true,
			"CSM",
			"Player State in slot " + passed_slot + " exists. Loading " + str(_player_state)
		)
			...
			if data == null:
				...
				for slot in player.inventory_data.inventory_slots:
					if slot and slot.inventory_item and slot.inventory_item == data["resource"]:
						CogitoGlobals.debug_log(
							true, "CSM", "Match found: " + str(slot.inventory_item)
						)
		...
		CogitoGlobals.debug_log(
			true, "CSM", "Player state of slot " + passed_slot + " doesn't exist."
		)
...
func save_player_state(player, slot: String):
	...
	for item_slot in player.inventory_data.inventory_slots:
		if (
			item_slot
			and item_slot.inventory_item
			and item_slot.inventory_item.has_method("update_wieldable_data")
		):  #Checking for wieldables.
			var item_save_data = item_slot.inventory_item.save()
			_player_state.append_saved_wieldable_charges(item_save_data)
			CogitoGlobals.debug_log(
				true, "CSM", "Saved charge for " + str(item_slot.inventory_item)
			)

	_player_state.player_current_scene = _current_scene_name
	CogitoGlobals.debug_log(
		true, "CSM", "Save_player_state(): setting player_current_scene to " + _current_scene_name
	)
	...
	_player_state.add_interaction_component_state_data_to_array(
		current_player_interaction_component.save()
	)
...
func load_scene_state(_scene_name_to_load: String, slot: String):
	CogitoGlobals.debug_log(
		true, "CSM", "Load scene state for:" + _scene_name_to_load + ". Slot: " + slot
	)
	if !_scene_state:
	...
	if _scene_state and _scene_state.state_exists(slot, _scene_name_to_load):
		...
		for node_data in array_of_node_data:
			...
			new_object.position = Vector3(
				node_data["pos_x"], node_data["pos_y"], node_data["pos_z"]
			)
			new_object.rotation = Vector3(
				node_data["rot_x"], node_data["rot_y"], node_data["rot_z"]
			)
			# Restore physics properties if it's a RigidBody3D
			if new_object is RigidBody3D:
				if (
					"linear_velocity_x" in node_data
					and "linear_velocity_y" in node_data
					and "linear_velocity_z" in node_data
				):
				...
				if (
					"angular_velocity_x" in node_data
					and "angular_velocity_y" in node_data
					and "angular_velocity_z" in node_data
				):
					new_object.angular_velocity = Vector3(
						node_data["angular_velocity_x"],
						node_data["angular_velocity_y"],
						node_data["angular_velocity_z"]
					)
			...
			if new_object.has_method("update_wieldable_data"):  #Check if item is wieldable
				CogitoGlobals.debug_log(
					true,
					"CSM",
					"Setting charge of " + new_object + " to " + node_data["item_charge"]
				)
		...
		for state_data in array_of_state_data:
			...
			# Set variables here
			node_to_set.position = Vector3(
				state_data["pos_x"], state_data["pos_y"], state_data["pos_z"]
			)
			node_to_set.rotation = Vector3(
				state_data["rot_x"], state_data["rot_y"], state_data["rot_z"]
			)
			for data in state_data.keys():
				if (
					data == "filename"
		...
		for node in save_nodes:
			if node.scene_file_path.is_empty():  # Check the node is an instanced scene so it can be instanced again during load.
				CogitoGlobals.debug_log(
					true,
					"CSM",
					"persistent node '%s' is not an instanced scene, skipped" % node.name
				)
				continue
			...
			if !node.has_method("save"):  # Check the node has a save function.
				CogitoGlobals.debug_log(
					true,
					"CSM",
					"persistent node '%s' is missing a save() function, skipped" % node.name
				)
				continue
...
func load_next_scene(
	...
	CogitoGlobals.debug_log(
		...
		"CSM",
		(
			"Loading screen initiated with: next_scene_path="
			+ target
			+ " | connector = "
			+ connector_name
			+ " | passed_slot = "
...
func copy_slot_saves_to_temp(passed_slot: String) -> bool:
	CogitoGlobals.debug_log(
		true, "CSM", "Attpemting to copy files from slot " + passed_slot + " to temp."
	)
	...
	if slot_dir:
		...
		while file_name != "":
			...
			if (
				slot_dir.copy(
					str(CogitoSceneManager.cogito_state_dir + passed_slot + "/" + file_name),
					str(CogitoSceneManager.cogito_state_dir + "temp/" + file_name),
					-1
				)
				!= OK
			):
...
func copy_temp_saves_to_slot(passed_slot: String) -> bool:
	CogitoGlobals.debug_log(
		true, "CSM", "Attpemting to copy files from temp to slot " + passed_slot
	)
	var files: Dictionary
	...
	if dir2:
		...
		while file_name != "":
			if dir2.current_is_dir():
				CogitoGlobals.debug_log(
					true, "CSM", "Deleting temp saves: Detected dir = " + file_name
				)
				if file_name == "temp":
	...
	if dir:
		...
		while file_name != "":
			if dir.current_is_dir():
				CogitoGlobals.debug_log(
					true, "CSM", "Found directory: " + file_name + ". skipping ahead."
				)
...
func fade_in(fade_duration: float = default_fade_duration) -> void:
	...
	fade_tween.tween_property(fade_panel, "modulate", Color.TRANSPARENT, fade_duration).set_trans(
		Tween.TRANS_CUBIC
	)
...
func fade_out(fade_duration: float = default_fade_duration) -> void:
	...
	fade_tween.tween_property(fade_panel, "modulate", Color.BLACK, fade_duration).set_trans(
		Tween.TRANS_CUBIC
	)
	await fade_tween.finished
	fade_finished.emit()
