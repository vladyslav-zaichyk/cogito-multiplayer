...
func _play_interaction(interaction_type: String):
	var query = PhysicsRayQueryParameters3D.create(
		global_position, global_position + Vector3(0, -1, 0)
	)
	...
	if result:
		...
		if interaction_type == "footstep":
			if _play_by_footstep_surface(result.collider, interaction_type):
				return
			elif _play_by_material(
				result.collider,
				footstep_material_library,
				generic_fallback_footstep_profile,
				interaction_type
			):
				return
			...
			if _play_by_landing_surface(result.collider, interaction_type):
				...
				result.collider,
				landing_material_library,
				generic_fallback_landing_profile,
				interaction_type
			):
				return
...
func _play_by_material(
	collider: Node3D,
	material_library: FootstepMaterialLibrary,
	fallback_profile: AudioStreamRandomizer,
	...
	if material_library:
		...
		if material:
			...
			var profile: AudioStreamRandomizer = material_library.get_footstep_profile_by_material(
				material
			)
		...
		if mesh.get_surface_count() == 0:
			...
			for i in range(len(faces) / 3):
				...
				if ray_t:
					...
					coord = [
						round(faces[face_idx] / snap),
						round(faces[face_idx + 1] / snap),
						round(faces[face_idx + 2] / snap)
					]
					break
			var mat = null
			if face:
