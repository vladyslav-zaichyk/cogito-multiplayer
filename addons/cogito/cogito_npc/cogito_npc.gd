...
func update_animations(_delta: float):
	var relative_velocity = (
		self.global_basis.inverse() * ((self.velocity * Vector3(1, 0, 1)) / sprint_speed)
	)
...
func head_lock_at(_delta: float) -> void:
	...
	bone_smooth_rotation = lerp_angle(
		bone_smooth_rotation, deg_to_rad(marker_rotation_deg.y), 3 * _delta
	)

	new_rotation = Quaternion.from_euler(
		Vector3(deg_to_rad(marker_rotation_deg.x), deg_to_rad(marker_rotation_deg.y), 0)
	)
