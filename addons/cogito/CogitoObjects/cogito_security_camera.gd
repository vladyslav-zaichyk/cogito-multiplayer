...
enum DetectionMethod { TIME, LIGHTMETER }  ##Uses Time directly to until detection is trigger  ##Uses Time * Lightmeter until detection is triggered. Scaled by light_level_detection_multipler.
...
enum DetectorState { SEARCHING, DETECTING, DETECTED, OFFLINE }
...
func detecting(delta: float):
	...
	if detected_objects.size() <= 0:
		CogitoGlobals.debug_log(
			true,
			"cogito_security_camera.gd",
			"DETECTING: No visible targets in detection area. Stopping detection."
		)
		stop_detecting()
...
func on_body_entered_detection(body: Node3D):
	if only_detect_player and body.is_in_group("Player"):
		CogitoGlobals.debug_log(
			true, "cogito_security_camera.gd", "Player entered detection area: " + body.name
		)
	...
	# For non-player objects, check they are in the non_player_detection_list, and not a player
		...
		if is_relevant_non_player(body):
			CogitoGlobals.debug_log(
				true,
				"cogito_security_camera.gd",
				"Relevant object entered detection area: " + body.name
			)
			objects_in_detection_area.append(body)
...
func find_lightmeter(body):
	...
	if found_lightmeter:
		...
		CogitoGlobals.debug_log(
			true, "cogito_security_camera.gd", "Lightmeter found on entity:" + body.name
		)
	else:
		CogitoGlobals.debug_log(
			true, "cogito_security_camera.gd", "No lightmeter found on entity:" + body.name
		)
...
func on_body_left_detection(body: Node3D):
	if only_detect_player and body.is_in_group("Player"):
		CogitoGlobals.debug_log(
			true, "cogito_security_camera.gd", "Player left detection area: " + body.name
		)
		objects_in_detection_area.erase(body)
	elif is_relevant_non_player(body):
		CogitoGlobals.debug_log(
			true, "cogito_security_camera.gd", body.name + " left detection area: "
		)
		objects_in_detection_area.erase(body)
