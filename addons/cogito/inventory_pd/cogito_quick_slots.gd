...
func set_inventory_quickslots(passed_inventory: CogitoInventory) -> void:
	if passed_inventory.assigned_quickslots.is_empty():
		CogitoGlobals.debug_log(
			true, "CogitoQuickSlots.gd", "set_inventory_quickslots: assigned_quickslots were empty."
		)
		...
		(
			...
			. debug_log(
				true,
				"CogitoQuickSlots.gd",
				"set_inventory_quickslots: assigned_quickslots were NOT empty, updating quickslot data..."
			)
		)
...
func unbind_quickslot(quickslot_to_unbind: CogitoQuickslotContainer) -> void:
	...
	if slot_index != -1:
		...
		CogitoGlobals.debug_log(
			true, "CogitoQuickSlots.gd", "quickslot wasn't found in the quickslot_container array."
		)
...
func bind_to_quickslot(
	itemslot_to_bind: InventorySlotPD, quickslot_to_bind: CogitoQuickslotContainer
):
	...
	if slot_index > -1:
		...
		CogitoGlobals.debug_log(
			true,
			"CogitoQuickSlots.gd",
			"Assigned quickslot " + str(slot_index) + " to " + itemslot_to_bind.inventory_item.name
		)
	else:
		CogitoGlobals.debug_log(
			true, "CogitoQuickSlots.gd", "quickslot wasn't found in the quickslot_container array."
...
func _unhandled_input(event):
	...
	if event.is_action_released("quickslot_1"):
		if inventory_reference.assigned_quickslots[0]:
			...
			CogitoGlobals.debug_log(
				true, "CogitoQuickSlots.gd", "Nothing assigned in quickslot 1..."
			)
			return
	...
	if event.is_action_released("quickslot_2"):
		if inventory_reference.assigned_quickslots[1]:
			...
			CogitoGlobals.debug_log(
				true, "CogitoQuickSlots.gd", "Nothing assigned in quickslot 2..."
			)
			return
	...
	if event.is_action_released("quickslot_3"):
		if inventory_reference.assigned_quickslots[2]:
			...
			CogitoGlobals.debug_log(
				true, "CogitoQuickSlots.gd", "Nothing assigned in quickslot 3..."
			)
			return
	...
	if event.is_action_released("quickslot_4"):
		if inventory_reference.assigned_quickslots[3]:
			...
			CogitoGlobals.debug_log(
				true, "CogitoQuickSlots.gd", "Nothing assigned in quickslot 4..."
			)
			return
...
func on_auto_quickslot_new_item(slot_data: InventorySlotPD) -> void:
		...
		if quickslot_containers[i].item_reference.name == slot_data.inventory_item.name:
			CogitoGlobals.debug_log(
				...
				(
					...
					+ " already in quickslot "
					+ str(quickslot_containers[i].get_index())
				)
			)
			return
	...
	for i in range(quickslot_containers.size()):
		...
		if (
			!quickslot_containers[i].inventory_slot_reference
			and !quickslot_containers[i].item_reference
		):
			if (
				slot_data.inventory_item.slot_number == -1
				or slot_data.inventory_item.slot_number == i + 1
			):
				...
				CogitoGlobals.debug_log(
					true,
					"CogitoQuickSlots.gd",
					"Auto-quickslotted " + slot_data.inventory_item.name
				)
				return
...
func _cycle_through_quickslotted_wieldables(cycle_up: bool) -> void:
	...
	if player_interaction_component.is_carrying:
		CogitoGlobals.debug_log(
			true, "CogitoQuickSlots.gd", "Can't cycle wieldables while carrying."
		)
		return

	if player_interaction_component.interactable:
		for node: InteractionComponent in (
			player_interaction_component.interactable.interaction_nodes
		):
			if node.input_map_action == "interact2" and not node.is_disabled:
				CogitoGlobals.debug_log(
					...
					"CogitoQuickSlots.gd",
					(
						"Looking at an interactable that"
	...
	var current_equipped_wieldable: WieldableItemPD = (
	...
	if current_wieldable_quickslot_index == -1:  # Not wielding
		if cycle_up:
			inventory_reference.use_slot_data(
				(
					inventory_reference
					. assigned_quickslots[quickslotted_wieldable_indexes[0]]
					. origin_index
				)
			)
		else:
			inventory_reference.use_slot_data(
				(
					inventory_reference
					. assigned_quickslots[quickslotted_wieldable_indexes[-1]]
					. origin_index
				)
			)
		return
	...
	if cycle_to_index != -1:
		inventory_reference.use_slot_data(
			inventory_reference.assigned_quickslots[cycle_to_index].origin_index
		)
