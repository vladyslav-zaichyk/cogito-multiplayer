...
func _on_focus_changed(control: Control):
	if !control.has_method("set_slot_data"):
		CogitoGlobals.debug_log(
			true, "inventory_interface.gd", "_on_focus_changed: Not a slot. returning."
		)
		return
	...
	if control_in_focus.item_data and !grabbed_slot_node.visible:
		...
		info_panel.global_position = (
			control_in_focus.global_position + Vector2(0, control_in_focus.size.y)
		)
...
func update_grabbed_slot_position():
	...
	grabbed_slot_node.global_position = (
		control_in_focus.global_position + (control_in_focus.size / 2)
	)
...
func on_inventory_button_press(
	inventory_data: CogitoInventory, index: int, action: String, local_inventory_ui
):
	match [grabbed_slot_data, action]:
		[null, "inventory_move_item"]:
			...
			if (
				temp_slot_data
				and temp_slot_data.inventory_item
				and temp_slot_data.inventory_item.is_being_wielded
			):
				Audio.play_sound(sound_error)
				CogitoGlobals.debug_log(
					true, "inventory_interface.gd", "Can't move item while its being wielded."
				)
			else:
		...
		[_, "inventory_use_item"]:
			print(
				"Inventory_interface.gd: Gamepad use_item pressed while grabbed_slot_data. Calling drop_single_slot_data..."
			)
		...
		[null, "inventory_drop_item"]:
			...
			if grabbed_slot_data:
				if !grabbed_slot_data.inventory_item.is_droppable:
					CogitoGlobals.debug_log(
						true, "inventory_interface.gd", "Item is not droppable."
					)
					Audio.play_sound(sound_error)
					grabbed_slot_data = null
					return
				if (
					grabbed_slot_data.inventory_item.has_method("update_wieldable_data")
					and grabbed_slot_data.inventory_item.is_being_wielded
				):
					#if grabbed_slot_data.inventory_item.ItemType.WIELDABLE and grabbed_slot_data.inventory_item.is_being_wielded:
					Audio.play_sound(sound_error)
					CogitoGlobals.debug_log(
						true, "inventory_interface.gd", "Can't drop while wielding this item."
					)
					grabbed_slot_data = null
				else:
					CogitoGlobals.debug_log(
						true, "inventory_interface.gd", "Dropping slot data via gamepad "
					)
					grabbed_slot_data = inventory_data.grab_single_slot_data(index)
					if not _drop_item(grabbed_slot_data):
						Audio.play_sound(sound_error)
						get_parent().player.player_interaction_component.send_hint(
							null, "Not enough space to drop item."
						)
						CogitoGlobals.debug_log(
							true,
							"inventory_interface.gd",
							"Can't drop because there isn't enough space."
						)
					else:
						grabbed_slot_data = null
		...
		[_, "inventory_drop_item"]:
			Audio.play_sound(sound_error)
			CogitoGlobals.debug_log(
				true, "inventory_interface.gd", "Can't drop while moving an item."
			)
...
func _on_bind_grabbed_slot_to_quickslot(quickslotcontainer: CogitoQuickslotContainer):
	if grabbed_slot_data:
		CogitoGlobals.debug_log(
			...
			"inventory_interface.gd",
			(
				"Binding to quickslot container: "
				+ str(grabbed_slot_data)
				+ " -> "
				+ str(quickslotcontainer)
			)
		)
		...
		match event.button_index:
			MOUSE_BUTTON_LEFT:
				if !grabbed_slot_data.inventory_item.is_droppable:
					Audio.play_sound(sound_error)
					CogitoGlobals.debug_log(
						true, "inventory_interface.gd", "This item isn't droppable."
					)
					return
...
func _drop_item(slot_data: InventorySlotPD) -> bool:
	...
	dropped_item.position = (
		shape_cast.global_position + (safe_drop_distance - item_length / 2) * -camera_basis_z
	)
	...
	if item_length < player_radius:
		...
		dropped_item.position = (
			...
			+ (safe_drop_distance - item_length / 2 + player_radius) * -camera_basis_z
		)
