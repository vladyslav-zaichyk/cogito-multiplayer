...
func _input(event):
	...
	if (
		InputHelper.device_index != -1
		and inventory_interface_reference
		and inventory_interface_reference.external_inventory_owner
	):
	...
	if event.is_action_pressed("ui_next_tab"):
		...
		if (
			InputHelper.device_index != -1
			and inventory_interface_reference != null
			and current_tab == tab_index_of_inventory
		):
		...
		if (
			InputHelper.device_index != -1
			and bindings_container_node != null
			and current_tab == tab_index_of_bindings
		):
...
func process_input_for_external_inventory_screen(event: InputEvent) -> void:
	if (event.is_action_pressed("ui_next_tab")) or (event.is_action_pressed("ui_prev_tab")):
		if focus_currently_on_external_inventory:
			...
			(
				inventory_interface_reference
				. external_inventory_ui
				. slot_array[0]
				. grab_focus
				. call_deferred()
			)
