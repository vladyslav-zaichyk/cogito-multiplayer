...
func _enter_tree() -> void:
	...
	CogitoGlobals.debug_log(
		true,
		"CogitoScene.gd",
		"Current scene root node set to " + CogitoSceneManager._current_scene_root_node.name
	)
...
func setup_bgm() -> void:
	...
	if not is_instance_valid(bgm_neutral):
		CogitoGlobals.debug_log(
			...
			"cogito_scene.gd",
			(
				'Scene Music is lacking a "bgm_neutral". Set it in the inspector on the node: {0}'
				. format([get_path()])
			)
		)
		valid = false
	if not is_instance_valid(bgm_combat):
		(
			...
			. debug_log(
				...
				"cogito_scene.gd",
				(
					'Scene Music Demo script is lacking a "bgm_combat". Set it in the inspector on the node: {0}'
					. format([get_path()])
				)
			)
		)
...
func music_fade_in(
	audio_stream_player, seconds := 1.0, max_volume_db := 0.0, tween := create_tween()
):
	if not (
