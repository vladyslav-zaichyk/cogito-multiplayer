...
var player_state_dir: String = (
	CogitoSceneManager.cogito_state_dir + CogitoSceneManager.cogito_player_state_prefix
)
...
func load_sitting_state(player):
	...
	if is_sitting and current_sittable_path != null:
		CogitoSceneManager._current_sittable_node = CogitoSceneManager.get_node(
			current_sittable_path
		)
...
func add_to_world_dictionary(world_property_name: String, world_property_data):
	if world_dictionary.has(world_property_name):
		...
		CogitoGlobals.debug_log(
			...
			"CogitoPlayerState",
			(
				"world dict key found and value saved: "
				+ str(world_property_name)
				+ " "
				+ str(world_property_data)
			)
		)
	else:
		...
		CogitoGlobals.debug_log(
			...
			"CogitoPlayerState",
			(
				"world dict key not found. Added and value saved: "
				+ str(world_property_name)
				+ " "
				+ str(world_property_data)
			)
...
func write_state(state_slot: String) -> void:
	...
	var player_state_file = str(
		(
			CogitoSceneManager.cogito_state_dir
			+ state_slot
			+ "/"
			+ CogitoSceneManager.cogito_player_state_prefix
			+ ".res"
		)
	)
	...
	CogitoGlobals.debug_log(
		true, "CogitoPlayerState", "Player state saved as " + str(player_state_file)
	)
