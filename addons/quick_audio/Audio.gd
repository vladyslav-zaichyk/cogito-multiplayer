...
func fade_in(audio_stream_player, seconds := 1.0, tween := create_tween()):
	if not (
		...
		return
	tween.tween_method(
		func(x): audio_stream_player.volume_db = linear_to_db(x),
		db_to_linear(audio_stream_player.volume_db),
		1.0,
		seconds
	)
...
func cross_fade(
	audio_stream_player_out, audio_stream_player_in, seconds := 1.0, tween := create_tween()
):
	if not (
		audio_stream_player_out is AudioStreamPlayer
		or audio_stream_player_out is AudioStreamPlayer2D
		or audio_stream_player_out is AudioStreamPlayer3D
	):
		push_error(
			"Non-AudioStreamPlayer[XD] provided to Audio.cross_fade(...) as audio_stream_player_out"
		)
		return
	if not (
		audio_stream_player_in is AudioStreamPlayer
		or audio_stream_player_in is AudioStreamPlayer2D
		or audio_stream_player_in is AudioStreamPlayer3D
	):
		push_error(
			"Non-AudioStreamPlayer[XD] provided to Audio.cross_fade(...) as audio_stream_player_in"
		)
		return
...
func sequential_fade(
	audio_stream_player_out,
	audio_stream_player_in,
	out_seconds := 1.0,
	...
	if not (
		...
		push_error(
			"Non-AudioStreamPlayer[XD] provided to Audio.sequential_fade(...) as audio_stream_player_out"
		)
		return
	if not (
		audio_stream_player_in is AudioStreamPlayer
		...
		push_error(
			"Non-AudioStreamPlayer[XD] provided to Audio.sequential_fade(...) as audio_stream_player_in"
		)
		return
